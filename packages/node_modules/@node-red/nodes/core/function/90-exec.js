...
module.exports = function(RED) {
    ...
    function ExecNode(n) {
        ...
        var cleanup = function(p) {
            ...
            if (node.activeProcesses[p].stdinpipe) { node.activeProcesses[p].stdinpipe.destroy(); node.activeProcesses[p].stdinpipe = undefined; }
            ...
            if (msg.hasOwnProperty("kill")) {
                ...
                if (msg.hasOwnProperty("pid")) {
                    if (node.activeProcesses.hasOwnProperty(msg.pid) ) {
                        let child = node.activeProcesses[msg.pid];
                        if (child.stdinpipe) { child.stdinpipe.destroy(); child.stdinpipe = undefined; }
                        child.kill(msg.kill.toUpperCase());
                        node.status({fill:"red",shape:"dot",text:"killed"});
                    }
                }
                else {
                    if (Object.keys(node.activeProcesses).length === 1) {
                        let child = node.activeProcesses[Object.keys(node.activeProcesses)[0]]; 
                        if (child.stdinpipe) { child.stdinpipe.destroy(); child.stdinpipe = undefined; }
                        child.kill(msg.kill.toUpperCase());
                        node.status({fill:"red",shape:"dot",text:"killed"});
                    }
                ...
                // get data for stdin pipe if available
                var stdindata = RED.util.getMessageProperty(msg, "stdin") || this.stdin;
                var stdinpipe = undefined;
                if (stdindata) {
                    // check for sanity of input data (only allow buffer or string types)
                    if (!Buffer.isBuffer(stdindata) && !(typeof stdindata === 'string') && !(stdindata instanceof String)) {
                        node.error("stdin input must be of type string or Buffer", RED.util.cloneMessage(msg));
                        nodeDone();
                        return;
                    }
                    stdinpipe = new stream.Readable();
                    stdinpipe.push(stdindata);  // add data to the internal queue for users of the stream to consume
                    stdinpipe.push(null);       // signals end of the stream (EOF)
                }
                ...
                if (this.useSpawn === true) {
                    ...
                    // pipe stdin
                    if (stdinpipe) {
                        stdinpipe.pipe(child.stdin);
                        child.stdinpipe = stdinpipe;
                    }
        ...
        this.on('close',function() {
            for (var pid in node.activeProcesses) {
                ...
                if (node.activeProcesses.hasOwnProperty(pid)) {
                    ...
                    if (node.activeProcesses[pid].stdinpipe) { node.activeProcesses[pid].stdinpipe.destroy(); node.activeProcesses[pid].stdinpipe = undefined; }
